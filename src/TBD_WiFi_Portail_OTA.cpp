...
TBD_WiFi_Portail_OTA::TBD_WiFi_Portail_OTA(TBD_WiFi_Portail_SerialDebug &serialDebug, TBD_WiFi_Portail_FileSystem &fileSystem, String hostname, String password) : _serialDebug(&serialDebug),
...
TBD_WiFi_Portail_OTA::~TBD_WiFi_Portail_OTA()
{
...
void TBD_WiFi_Portail_OTA::addWebEvents(TBD_WiFi_Portail_WebEvents &webEvents)
{
    this->_webEvents = &webEvents;
}

void TBD_WiFi_Portail_OTA::addMDNS(TBD_WiFi_Portail_MDNS &mdns)
{
    this->_mdns = &mdns;
}

void TBD_WiFi_Portail_OTA::setHostname(const String &hostname)
{
...
void TBD_WiFi_Portail_OTA::setPassword(const String &password)
{
    this->_password = password;
}

String TBD_WiFi_Portail_OTA::getHostname() const
{
    return this->_hostname;
}

String TBD_WiFi_Portail_OTA::getPassword() const
{
    return this->_password;
}

void TBD_WiFi_Portail_OTA::setArduinoOtaUseMDNS(bool arduinoOtaUseMDNS)
{
    this->_arduinoOtaUseMDNS = arduinoOtaUseMDNS;
}

bool TBD_WiFi_Portail_OTA::getArduinoOtaUseMDNS() const
{
    return this->_arduinoOtaUseMDNS;
}

String TBD_WiFi_Portail_OTA::toString() const
{
    return (String)F("=========== OTA ===========") + F("\n") + F("Hostname :      ") + this->getHostname() + F("            !!!! used mDNS hostname !!!!") + F("\n") + F("Password :      ") + this->getPassword() + F("\n") + F("===========================");
}

// ------------------------------------ setup OTA ----------------------------------------
void TBD_WiFi_Portail_OTA::begin()
{
    ...
    // Authentication
    ArduinoOTA.setPassword(this->_password.c_str()); //IDE Arduino nous demandera de saisir le mot de passe à chaque fois que vous téléverserez un programme.
    ...
    ArduinoOTA.onProgress(
        ...
        { this->arduinoOTAOnProgress(progress, total); });
    ArduinoOTA.onError(
        [&](ota_error_t error)
        { this->arduinoOTAOnError(error); });

    ArduinoOTA.begin(this->_arduinoOtaUseMDNS); // initialize OTA

    this->_serialDebug->println(F(" -> OTA ready"));
    this->_serialDebug->println(F("/!\\ Be shure to be in the same network for ota update (computer self-sharedWifi not works !!!!)"));
    this->_serialDebug->println(F("================="));
}

// ---------------------------------------- loop OTA -------------------------------------------
void TBD_WiFi_Portail_OTA::loop()
{
...
void TBD_WiFi_Portail_OTA::rebootESP8266()
{
...
void TBD_WiFi_Portail_OTA::reset_all_pins()
{
...
void TBD_WiFi_Portail_OTA::arduinoOTAOnStart()
{
    this->_serialDebug->println(F("Be shure to be in the same network for ota update (computer self-sharedWifi not works !!!!)"));
    if (this->_webEvents != nullptr)
        this->_webEvents->debug_to_WSEvents(F("Be shure to be in the same network for ota update (computer self-sharedWifi not works !!!!)"), "ota");
...
void TBD_WiFi_Portail_OTA::arduinoOTAOnEnd()
{
...
void TBD_WiFi_Portail_OTA::arduinoOTAOnProgress(unsigned int progress, unsigned int total)
{
    ...
    if (millis() - this->previousMillisSendInfos >= this->intervalSendInfos)
    {
        ...
        char p[80];
        sprintf(p, "* OTA: Progress: %u%% | free space: %u / total: %u\n", (progress / (total / 100)), ESP.getFreeSketchSpace(), (ESP.getSketchSize() + ESP.getFreeSketchSpace()));
        //events.send(p, "ota");
        if (this->_webEvents != nullptr)
            this->_webEvents->debug_to_WSEvents(p, "ota");
    }
...
void TBD_WiFi_Portail_OTA::arduinoOTAOnError(ota_error_t error)
{
    if (this->_webEvents != nullptr)
    {
