...
TBD_WiFi_Portail_WebEvents::TBD_WiFi_Portail_WebEvents(TBD_WiFi_Portail_SerialDebug& serialDebug, TBD_WiFi_Portail_WebServer& webServer, String eventsRoot, String eventsAdminRoot): _serialDebug(&serialDebug), _webServer(&webServer), _eventsRoot(eventsRoot), _eventsAdminRoot(eventsAdminRoot) {

    this->events = new AsyncEventSource(_eventsRoot);
    this->eventsAdmin = new AsyncEventSource(_eventsAdminRoot);
...
void TBD_WiFi_Portail_WebEvents::setEventsAdmin_UserPass(String user, String password)
{
...
void TBD_WiFi_Portail_WebEvents::begin() // Start a WebEvents server
{
    ...
    this->events->onConnect([&](AsyncEventSourceClient *client){
        if(client->lastId()){
            this->_serialDebug->print(F("[Events] Client reconnected! Last message ID that it gat is: "));
    ...
    this->eventsAdmin->onConnect([&](AsyncEventSourceClient *client){
        if(client->lastId()){
            this->_serialDebug->print(F("[EventsAdmin] Admin client reconnected! Last message ID that it gat is: "));
            this->_serialDebug->println((String)client->lastId());
        }
        //send event with message "hello!", id current millis
        // and set reconnect delay to 1 second
    ...
    this->_serialDebug->printf(F(" -> EventsAdmin server started. ( %s ) user: %s / pass: %s \n"), this->_eventsAdminRoot.c_str(), this->_adminUser, this->_adminPass);

    this->_serialDebug->println(F("====================="));
}

// --------------------------------- send messsage to debug via websocket events ---------------------------------------
void TBD_WiFi_Portail_WebEvents::debug_to_WSEvents(String message, String type, int niveau){
