...
namespace WiFi_Portail_API {
    ...
    WebEvents::WebEvents(SerialDebug &serialDebug, WebServer &webServer, const String &eventsRoot, const String &eventsAdminRoot) :
            _serialDebug(&serialDebug), _webServer(&webServer), _eventsRoot(eventsRoot), _eventsAdminRoot(eventsAdminRoot) {
    ...
    void WebEvents::begin() // Start a WebEvents server
    {
        ...
        this->eventsAdmin->onConnect([&](AsyncEventSourceClient *client) {
            ...
            debug_to_WSEvents((String) F("New Admin client connected ") + (String) client->lastId(),
                              this->eventType->debug);
        });
